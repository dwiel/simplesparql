...
class Evaluator :
	...
	def evaluate_helper_old(self, compile_node, incoming_bindings_set, modifiers) :
		"""
		...
		for incoming_bindings in incoming_bindings_set :
			...
			for step in compile_node['guaranteed'] :
				...
				# recur
				rets.extend(self.evaluate_helper_old(step, new_bindings, modifiers))
				# TODO: this isnt quite right on recured cases ... It will return (and
				# evalute) more than is necessary.  I think ...
				if len(rets) >= modifiers['limit'] :
					return rets[:modifiers['limit']]
		#print 'rets',prettyquery(rets)
		return rets
		
	def evaluate_old(self, compile_node, incoming_bindings_set = [{}]) :
		modifiers = {}
		...
		return self.evaluate_helper_old(compile_node, incoming_bindings_set, modifiers)
